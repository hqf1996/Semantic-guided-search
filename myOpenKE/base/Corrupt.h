...
bool _findFromTrain(INT h, INT t, INT r){
    INT low = 0;
    INT high = trainTotal - 1;
    INT mid;
    while (low + 1 < high){
        INT mid = (low + high) >> 1;
    ...
    if(trainList[low].h == h && trainList[low].r == r && trainList[low].t == t)
        return true;
    if(trainList[high].h == h && trainList[high].r == r && trainList[high].t == t)
        return true;
    return false;
}
