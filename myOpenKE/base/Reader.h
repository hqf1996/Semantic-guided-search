...
INT *testLef, *testRig; //testdList中按照边id排序后，边在validList的起始index存放在testLef，结尾index存放在testRig
INT *validLef, *validRig;  //同上，对vaildList处理结果
...
//测试数据，一维数组，记录了头结点，边，尾结点的id的Triple结构体，按照边的序号排序
Triple *testList;
//记录了vaild的头结点，边，尾结点的id的Triple结构体，按照边的序号排序
Triple *validList;
//记录了test,train,vaild的所有头结点，边，尾结点的id的Triple结构体,按照头结点的序号排序
Triple *tripleList;
